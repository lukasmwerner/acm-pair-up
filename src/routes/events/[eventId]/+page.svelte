<script lang="ts">
	...
	import { connectSSE, loadSession, startHeartbeat } from '$lib/client/session';

	type Summary = {
		event: { id: string; name: string; code: string };
		participants: Array<{ id: string; hexId: string; displayName?: string; presence?: { connected: boolean } }>;
	...
	function bgFromHex(hex: string) {
		...
		let val = 0;
		try { val = parseInt(hex, 16) % 360; } catch {}
		...
		if (data.pairings) {
			for (const p of data.pairings) {
				if (p.aParticipantId === s.participantId) {
					const other = summary?.participants.find(x => x.id === p.bParticipantId);
					if (other) partner = { hexId: other.hexId, displayName: other.displayName };
				}
				if (p.bParticipantId === s.participantId) {
					const other = summary?.participants.find(x => x.id === p.aParticipantId);
					if (other) partner = { hexId: other.hexId, displayName: other.displayName };
				}
			}
		}
	...
	onMount(async () => {
		...
		disconnectSSE = connectSSE(eventId, async (msg) => {
			if (msg?.type === 'presence:update' || msg?.type === 'pairings:updated') {
				await fetchState(eventId);
				await fetchPairings(eventId);
			}
		});
...
{#if summary}
	...
	<div class="fixed top-3 inset-x-0 z-20 flex justify-center">
		<div class="rounded-full bg-white/80 px-4 py-1 shadow">
			<span class="font-mono text-2xl text-slate-800">{my.hexId}</span>
		</div>
	</div>

	<!-- Fullscreen partner area with derived background -->
	<section class="fixed inset-0 z-10 flex items-center justify-center" style="background-color: {partnerBg};">
		{#if partner}
			<div class="text-center">
				<div class="font-mono text-[14vw] leading-none md:text-[10rem] text-slate-900">{partner.hexId}</div>
				{#if partner.displayName}
					<div class="mt-2 text-slate-700 text-xl">{partner.displayName}</div>
				{/if}
			</div>
		{:else}
			<div class="text-center">
				<p class="text-slate-700 text-xl">Waiting for pairing...</p>
				<p class="text-slate-600 text-sm mt-1">An organizer may rematch at any time.</p>
			</div>
		{/if}
	</section>

	<!-- Bottom overlay: event info -->
	<div class="fixed bottom-3 inset-x-0 z-20 text-center text-xs text-slate-600">
